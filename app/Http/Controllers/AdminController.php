...
class AdminController extends Controller
{
    function index(){
        ...
        //Sub users display
        $dawn_users = DB::table('sub_users')->where('channel', 'DAWN')->get();
        $ary_users = DB::table('sub_users')->where('channel', 'ARY')->get();
        $tribune_users = DB::table('sub_users')->where('channel', 'TRIBUNE')->get();

        return view('dashboards.admins.index' , compact('users', 'eng_news_count', 'urdu_news_count' , 'total_pos_sentiment', 'total_neg_sentiment', 'total_neu_sentiment', 'dawn_users', 'ary_users', 'tribune_users'));
       }
       ...
       public function delete($id){
        ...
        $id = DB::table('sentiment_results')->pluck('id')->sortByDesc('id')->take(20);
        $sentiments_english = DB::table('sentiment_results')->pluck('sentiments')->sortByDesc('id')->take(20);
        // $sentiments_english = $english->pluck('sentiments');
        // get scraped_data from sentiment_results table
        $scraped_data_english = DB::table('sentiment_results')->pluck('scraped_data')->sortByDesc('id')->take(20);
        ...
        // get scraped_data from sentiment_results_urdus table
        $scraped_data_urdus = DB::table('sentiment_results_urdus')->pluck('scraped_data')->sortByDesc('id')->take(20);
        return view('dashboards.admins.manage_results', compact('sentiments_english' , 'scraped_data_english', 'sentiments_urdus', 'scraped_data_urdus', 'id'));
        }
            
        

        public function deleteAll(Request $request){
            DB::table('sentiment_results')->delete();
            DB::table('sentiment_results_urdus')->delete();
            toastr()->success('All Results Deleted', 'Success');
            return redirect()->back();            
        }

        public function deleteResultEnglish(request $request , $id){
            // $id = $request->id;
            DB::table('sentiment_results')->where('id', $id)->delete();
        ...
        public function deleteSelective(Request $request , $id){
            
            // $ids = $request->ids;
            DB::table("sentiment_results")->whereIn('id',explode(",",$id))->delete();
            DB::table("sentiment_results_urdus")->whereIn('id',explode(",",$id))->delete();
    ...
    public function mailer(){
        ...
        // Toastr::success('Mail Sent Successfully', 'Success', ["positionClass" => "toast-top-right"]);
